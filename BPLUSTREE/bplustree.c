...
#include "bplustree.h"
node * queue = NULL;
int order = 4;//defined as the maximal number of pointers in any node
...
void find_and_print_range(node * const root, int key_start, int key_end,
    ...
    int *returned_keys = calloc(1, sizeof(int)*array_size);
    void ** returned_pointers = calloc(1, sizeof(void*) * array_size);
...
node * make_node(void)
{
    ...
    if(new_node->keys == NULL){
        perror("Keys Creation.");
        exit(EXIT_FAILURE);
    }
    new_node->pointers = malloc(order* sizeof(void*));
    if(new_node->pointers == NULL){
        perror("Pointers Creation.");
        exit(EXIT_FAILURE);
    }
...
node * insert_into_leaf(node * leaf, int key, record * pointer){
    ...
    for(insertion_point=0; insertion_point<leaf->num_keys&&leaf->keys[insertion_point] < key; insertion_point++);
...
node * insert_into_leaf_after_splitting(node * root, node * leaf, int key, record * pointer)
{
    ...
    if(temp_keys == NULL){
        perror("Temp keys creation.");
        exit(EXIT_FAILURE);
    }
    temp_pointers = malloc(order * sizeof(void*));
    if(temp_pointers == NULL){
        perror("Temp pointers creation.");
        exit(EXIT_FAILURE);
    }
    insertion_index = 0;
